...
map(names(refs), function(.name) {
  ...
  tags$details(
    ...
    p(gsub("\\[.*\\] ", "", paste(capture.output(ref), collapse = "\n")))
  )
}) %>% htmltools::as.tags()
```
::: {#refs}
...
items <- yaml::read_yaml("conferences.yaml") %>% 
  pluck("assorted")
for (item in items) {
cat("::: {.g-col-4 width='100%' height='auto' style='background: #f0f0f0;'}\n")

cat(glue::glue("![](images/{item$img})"), "\n")

cat(":::\n")

cat("::: {.g-col-8 width='100%' height='auto' style='background: #f0f0f0; padding: 10px 15px;'}\n")
